@php($hero = model(new \model\homepage\hero)->label('Hero'))
<div class="flex items-center justify-center bg-white dark:bg-gray-900 pt-[70px]">
    ...
    >
        ...
        <div class="md:w-1/2 mt-8 md:mt-0 md:ml-14 relative">
            <div @click="toggleShowVideo" class="opacity-0 transition duration-[1500ms]"
...
<div x-auto-animate>
    <template x-if="videoIsOpen">
        <div @keydown.escape="toggleShowVideo" id="popup-modal"
             class="fixed top-0 left-0 right-0 z-50 overflow-x-hidden overflow-y-auto md:inset-0 h-screen max-h-full bg-black/80 flex items-center justify-center">
            <div class="relative rounded-xl p-4">
                <div class="relative bg-white rounded-xl shadow p-2 h-5/6 dark:bg-gray-700 aspect-video">
                    <div class="absolute right-[-17px] -top-5 w-11 h-11 rounded-full bg-white cursor-pointer"
                    ...
                    <iframe class="rounded-xl w-full h-full aspect-video"
                            src="https://www.youtube.com/embed/TmWIrBPE6Bc" title="YouTube video player" autoplay=1
                            frameborder="0"
